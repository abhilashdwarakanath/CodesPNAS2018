...
datasetpaths{3} = 'E:\Data\H07\20161022\PFC\OriDisk1\SUSpikesByTime.mat';
datasetpaths{4} = 'E:\Data\A11\20161130\PFC\OriDisk1\SUSpikesByTime.mat';
datasetpaths{5} = 'E:\Data\A11\20161203\PFC\OriDisk1\SUSpikesByTime.mat';
datasetpaths{6} = 'E:\Data\A11\20161122\PFC\OriDisk1\SUSpikesByTime.mat';
...
%sitePairs = nchoosek(1:nunits,2);

sc = SU(i).signalCorrelation;
sc = triu(sc);
sc(sc==1) = NaN;
sc(sc==0) = NaN;

[x,y] = find(sc>0.1 & sc<0.9);
...
%pvalsTr = pValue(i).stim;

pvalsTr = 0.001.*ones(1,nunits);
corrsTrial{i} = sccgparnewTuned(params,SUspikesByTimeT,sitePairs,pvalsTr);
%pvalsItr = pValue(i).interTrial;
%corrsInterTrial{i} = sccgparnewITTuned(params,SUspikesByTimeIT,sitePairs,pvalsItr);

clear pvalsTr; clear pvalsItr;
...
%save('AwakeCorrs_TunedOnly.mat','cell_pairs','corrsTrial','corrsInterTrial','datasetpaths','pValue','dataInfo','utahMaps');

%% Plot

dBins = 0.5:0.5:4;
lags = linspace(-0.25,0.25,501);
...
fignames{3} = ['H07_22102016'];
fignames{4} = ['A11_30112016'];
fignames{5} = ['A11_03122016'];
fignames{6} = ['A11_22112016'];
...
subplot(2,2,[2 4])
for j = 1:8
    peaks(j) = max(abs(dataSetBinCorrs{i}(j,:)));
end
plot(dBins,peaks)
grid on
axis tight
...
print('-dpng', '-r600', strcat([fignames{i} 'HighSigCorrPairs_StructSynchrony_absMax'],'.png'));
end
