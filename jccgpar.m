function [ccJC] = jccgpar(params,spikeTimes,cell_pair,jitwin,i,tag)
...
for r = 1:size(cell_pair{i},1) % Run through all pairs in a particular distance bin
        ...
        case 0 %stim off
            ...
            for j = 1:size(spikeTimes,2)
                ...
                if numel(spikeTimes{cell_pair{i}(r,2),j}(spikeTimes{cell_pair{i}(r,2),j}>params.fulltime))~=0
                    ...
                    st2(sts2) = 1;
                    st3 = jitter2(st2,@randn,jitwin); % Corrected
                    st3 = st3';
                end
                spikeTrains{1}(:,j) = st1;
                spikeTrains{2}(:,j) = st2;
    ...
    for tt = 1:params.numtrials
        ...
        frb(:,tt) = spikeTrains{2}(:,tt);
        frbsum(tt) = sum(frb(:,tt));
        
        frc(:,tt) = spikeTrains{3}(:,tt); % Jittered
    ...
    for tt = 1:params.numtrials
        [cc1(:,tt),lags] = cross_corr(fra(:,tt),frb(:,tt),params.jmaxlag,1); % Jittered
        [cc2(:,tt)] = cross_corr(fra(:,tt),frc(:,tt),params.jmaxlag,1); % Expected CCG
        differ(:,tt) = cc1(:,tt)-cc2(:,tt); %#ok<*PFOUS> % Compute the jitter corrected CCG i.e. rawCCG-jitteredCCG
    ...
    if isempty(corr_cc1) ~=1
        ...
        ccJC(:,r) = corr_cc/(geomean([frassum frbssum]));
        ccJC(:,r) = ccJC(:,r)'./theta;
    else
        ccJC(:,r) = nan(2*params.jmaxlag+1,1);
    end
