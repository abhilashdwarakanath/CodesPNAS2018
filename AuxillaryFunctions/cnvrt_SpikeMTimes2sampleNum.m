function spkTimesSN = cnvrt_SpikeMTimes2sampleNum(data, favSpikeField, varargin)
% spkTimesSN = cnvrt_SpikeMTimes2sampleNum(data, favSpikeField, maxNumUnit)
...
% (1) It might be better to have input the whole data
% (2) should make some IF weather the input file is cell is mat?
...
for iTr = 1 : nTrials
    ...
    for channelNum = 1 : nUnit        
%         tmpRes = data.allSpikesByTime{channelNum,1}{iTr}(:, 1);   
        tmpRes = spikeTrainS{channelNum,1}{iTr}(:, 1); 
        ...
        spkTimesSN{channelNum, iTr}(:, 1) = DSndx;        
    end
end
