...
params.intwin = [1 2 5 10 25 50 125 250 500 750 1000 2500 5000]; % for computing r_ccg
...
params.firingthreshold = 10; % Mathematically, one needs 1 spike in a timeseries to compute a crosscorrelogram
...
for i = 1:params.datsets
    ...
    spikeTime = jMU(1,i).spikesBySampleIndex;
    ...
    MU_PWdistances = jMU(1,i).PWdistances; % distances
    
    MUcontam = utahMaps(1,i).MU.integratedTabel(:,5); % contamination indices
    ...
    SUind = find(MUcontam<=params.contamthreshold);
...
resultsOF.sjcorr = OFFresultsSJC;
resultsOFF.jittercorr = OFFresultsJC;
resultsOFF.spikecoher = OFFresultsSSC;

save('allMU_results_ON_1Hz_f0.1-150.mat','resultsON', '-v7.3')
save('allMU_results_OFF_1Hz_f0.1-150.mat','resultsOFF', '-v7.3')
